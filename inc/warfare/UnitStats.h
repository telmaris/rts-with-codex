...
// Per-tick CONTINUOUS upkeep for a division's three supply pools. Consumption
// scales with `strength/maxStrength` and the division's posture:
//   food     — always eaten; combat > marching > holding in the field > garrison.
//   materiel — equipment maintenance; a field/combat cost, negligible in garrison.
//   weapons  — spent in battle; light wear while marching in the field, else static.
// `deployed` = out on the map (vs. sitting in a building); `engaged` = fighting
// (unused while the combat system is removed — nothing sets it in the sim, but the
// combat rates are retained for when it returns). Sub-1-per-tick amounts accumulate
// in per-pool buffers so slow drains are not lost to rounding. When `foodSupply`
// hits zero the division starves and `strength` drains (manpower attrition).
// `supplyConservation` (from tech/focus/state — see PlayerSupplyConservation) scales
// down how much is burned; it is capped below 1.0 so upkeep is never free.
