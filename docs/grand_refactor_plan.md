...
- [~] Etap 4 — InputManager (observer) — 4.1 zrobione i poprawione (2026-07-05: naprawiono bug RAII — singleton + raw-pointer subscribers zamiast kopiującego shared_ptr), 4.2 zrobione (KeyBindingMap). 4.3 częściowo (2026-07-05, commit 759257e): KAŻDE bezpośrednie wywołanie raylib input API (IsKeyPressed/IsKeyDown/IsKeyReleased/IsMouseButtonPressed/IsMouseButtonDown/IsMouseButtonReleased/GetMouseWheelMove/IsKeyPressedRepeat) w całym repo zmigrowane na statyczne wrappery `InputManager::*` — zweryfikowane grepem: zero wystąpień poza InputManager.h/.cpp. To spełnia dosłownie kryterium akceptacji "jedyne wywołania raylibowego API inputu są w InputManager".
  - NIE zrobione: pełne zastąpienie `InputProcessor`/`actionMap`/`Input.h` przez subskrybentów per-widget. Powód: `GuiController::MakeAction` działa przez `activeSystem->actionMap[action]` — ten sam fizyczny klawisz (np. Q) znaczy różne rzeczy w różnych aktywnych systemach (BasicMapViewSystem vs BuildGuiSystem), więc statyczna rejestracja subskrybentów na stałe (jak dziś zrobione dla `escClose`) spowodowałaby, że WSZYSTKIE systemy reagowałyby na klawisz jednocześnie zamiast tylko aktywny. Poprawne zrobienie tego wymaga dynamicznego rejestrowania/wyrejestrowywania subskrybentów przy `ChangeSystem()` (Activate()/Deactivate() per GuiSystem) — real architectural change, nie mechaniczny rename, wymaga manualnego smoke-testu przełączania trybów (build/road/destroy/basic) w grze.
  - 4.4 (test manualny wszystkich skrótów) nie do zrobienia bez uruchomienia okna gry.
